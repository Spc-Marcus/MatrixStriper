...
logging.basicConfig(
    level=logging.INFO,  # ou DEBUG selon ton besoin
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("logs.txt", mode='w')
        # Pas de StreamHandler() ici !
    ]
)

# Ensuite seulement, les imports de modules qui font du logging
from MatrixStriper.test_pipelineILP import run_batch_test
...
def main():
    ...
    parser.add_argument('--max-matrices-per-folder', type=int, default=20,
                       help='Nombre maximum de matrices à traiter par dossier (default: 20)')
    ...
    logger.info(f"Paramètres: min_col_quality={args.min_col_quality}, "
                ...
                f"max_matrices_per_folder={args.max_matrices_per_folder}")
    logger.info("Note: Les résultats seront sauvegardés au fur et à mesure dans le CSV")
    ...
    try:
        ...
        results_df = run_batch_test(
            ...
            max_matrices_per_folder=args.max_matrices_per_folder
        )
